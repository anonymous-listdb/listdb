...
class CoreWorkload {
 public:
  ...
  ///
  virtual void Init(const utils::Properties &p, const bool is_load = false);
  
  typedef std::pair<std::string, std::string> KVPair;

  virtual void BuildValues(std::vector<KVPair> &values);
  virtual void BuildUpdate(std::vector<KVPair> &update);
  
  virtual std::string NextSequenceKey(); /// Used for loading data
  virtual std::array<std::string, 10> NextSequenceKeys();
  ...
  int field_count() { return field_count_; }
  size_t field_len() { return field_len_; }
  size_t record_count() { return record_count_; }
  size_t operation_count() { return operation_count_; }
  ...
  CoreWorkload() :
      ...
      ordered_inserts_(true), record_count_(0), operation_count_(0) {
  }
...
inline std::array<std::string, 10> CoreWorkload::NextSequenceKeys() {
  uint64_t key_num = key_generator_->NextN(10);
  std::array<std::string, 10> keys;
  for (uint64_t i = 0; i < 10; i++) {
    keys[i] = std::move(BuildKeyName(key_num+i));
  }
  //return BuildKeyName(key_num);
  return keys;
}
