...
void CoreWorkload::Init(const utils::Properties &p, const bool is_load) {
  name_str_ = p.GetProperty("workloadname");
  operation_count_ = std::stoull(p.GetProperty(OPERATION_COUNT_PROPERTY));
  is_load_ = is_load;

  field_count_ = std::stoi(p.GetProperty(FIELD_COUNT_PROPERTY,
                                         FIELD_COUNT_DEFAULT));
  field_len_ = std::stoi(p.GetProperty(FIELD_LENGTH_PROPERTY,
  ...
  record_count_ = std::stoull(p.GetProperty(RECORD_COUNT_PROPERTY));
  std::string request_dist = p.GetProperty(REQUEST_DISTRIBUTION_PROPERTY,
                                           REQUEST_DISTRIBUTION_DEFAULT);
  distribution_str_ = request_dist;
  distribution_str_[0] = toupper(distribution_str_[0]);
...
void CoreWorkload::BuildValues(std::vector<KVPair> &values) {
  for (int i = 0; i < field_count_; ++i) {
    KVPair pair(std::string("field" + std::to_string(i)),
                std::string(field_len_generator_->Next(), '0'));
    //          std::string(field_len_generator_->Next(), utils::RandomPrintChar()));
    values.push_back(std::move(pair));
  }
}

void CoreWorkload::BuildUpdate(std::vector<KVPair> &update) {
  KVPair pair;
  pair.first.append(NextFieldName());
  //pair.second.append(field_len_generator_->Next(), utils::RandomPrintChar());
  pair.second.append(field_len_generator_->Next(), '0');
