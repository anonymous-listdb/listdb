#ifndef WORKLOADWRAPPER_H
#define WORKLOADWRAPPER_H
#include <functional>
#include "ycsb/core/workload_proxy.h"
...
class WorkloadWrapper{
  public:
    ...
    void produce_request(const uint64_t num, bool is_load, bool master){
      ...
      for(uint64_t i=0; i<num; i++){
        ...
        if(is_load){
          ...
          requests_[index] = new Request(INSERT, key, value_len);
          continue;
        }
        opt = workload_proxy_->GetNextOperation();
        if(opt == READ){
          workload_proxy_->GetReadArgs(key);
          requests_[index] = new Request(opt, key, 0);
        }else if(opt == UPDATE){
          workload_proxy_->GetUpdateArgs(key, value_len);
          requests_[index] = new Request(opt, key, value_len);
        }else if(opt == INSERT){
          workload_proxy_->GetInsertArgs(key, value_len);
          requests_[index] = new Request(opt, key, value_len);
        }else if(opt == READMODIFYWRITE){
          workload_proxy_->GetReadModifyWriteArgs(key, value_len);
          requests_[index] = new Request(opt, key, value_len);
        }else if(opt == SCAN){
          workload_proxy_->GetScanArgs(key, len);
    ...
    Request** GetNextRequests() {
      uint64_t index = __sync_fetch_and_add(&cursor_, 10);
      return &requests_[index];
    }
